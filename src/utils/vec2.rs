//! Vector math, stolen from evian.
//!
//! Let's wait until the ecosystem has a decent vector math library.
...
impl<T: Default + Add<Output = T>> Sum for Vec2<T> {
    fn sum<I: Iterator<Item = Vec2<T>>>(iter: I) -> Self {
        iter.fold(Vec2::default(), Add::add)
    }
}
